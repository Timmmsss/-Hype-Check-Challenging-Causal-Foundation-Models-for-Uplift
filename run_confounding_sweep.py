...
from baseline_benchmark.data import _make_preprocessor  # after rr import (path side effect)

CONFOUNDING_AXIS = AxisSpec(
    name="confounding",
    ...
    extra_models=(),
    description=(
        "Covariate-dependent P*(T=1|C): converts the RCT into a confounded/"
        "observational sample (Keith et al. Algorithm 1, general case). "
        "Evaluation split stays a clean, unconfounded RCT."
    ),
)


def make_confounding_fn(strength: float, seed: int):
...
def patched_prepare_cell(args, dataset, axis, severity, seed):
    ...
    confounding_fn = make_confounding_fn(float(severity), seed=seed * 1000 + int(float(severity) * 100))
...
def main():
    ...
    for dataset in DATASETS:
        for strength in STRENGTHS:
            for seed in SEEDS:
                print(f"=== {dataset} confounding_lam{strength:.2f} seed {seed} ===", flush=True)
                try:
                    rr.run_cell(args, dataset, CONFOUNDING_AXIS, strength, seed, MODELS, {}, stamp)
                except Exception as exc:
                    print(f"CELL FAILED {dataset}/confounding/{strength}/seed_{seed}: {exc}", flush=True)
                    traceback.print_exc()
